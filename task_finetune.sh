...
gamma=$6
nsteps=$7
accelerate launch --config_file accelerate_configs/accelerate_config_zero2.yaml --main_process_port 59997 mmrec.py \
...
--run_name="mmrec-single-${task}-output-prefix-reweight-${gamma}-${nsteps}-answer-nonsemantic-${lr}-constant-${modelname}-b${bsz}-filter8-${subset}" \
